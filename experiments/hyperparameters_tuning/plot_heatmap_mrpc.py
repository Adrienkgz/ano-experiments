...
def get_nested_metric(data, key_path):
    """Accède à une valeur imbriquée dans un dictionnaire via un chemin de clés séparées par des points."""
    keys = key_path.split('.')
    current_val = data
    for key in keys:
...
def extract_metric(path, metric, *, maximize: bool, use_last: bool):
    ...
    # Utilise la nouvelle fonction pour trouver les valeurs
    vals = []
    for ep in data:
        val = get_nested_metric(ep, metric)
        if val is not None:
            vals.append(float(val))

    if not vals:
        # L'erreur est plus explicite si on indique le chemin complet
        raise KeyError(f"La métrique '{metric}' est absente de {path} ou son chemin est incorrect.")
...
def main():
    LOGDIR = "experiments/hyperparameters_tuning/logs/mrpc"
    OUTDIR = "experiments/hyperparameters_tuning/heatmaps"
    OPTIMIZERS = ["AdamW", "Ano", "Adan", 'Grams']

    METRIC = "aggregated_metric.Acc+F1" 
    MAXIMIZE = True
    USE_LAST = False
